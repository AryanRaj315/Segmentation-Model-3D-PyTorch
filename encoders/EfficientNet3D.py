...
class Conv3dDynamicSamePadding(nn.Conv3d):
    ...
    def forward(self, x):
        ...
        if pad_h > 0 or pad_w > 0 or pad_z > 0:
            x = F.pad(x, [pad_z // 2, pad_z - pad_z // 2, pad_w // 2, pad_w - pad_w // 2, pad_h // 2, pad_h - pad_h // 2]) ## Check
...
class Conv3dStaticSamePadding(nn.Conv3d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, image_size=None, **kwargs):
        ...
        self.pad_z = max((oz - 1) * self.stride[2] + (kz - 1) * self.dilation[2] + 1 - iz, 0)

    def forward(self, x):
        if self.pad_h > 0 or self.pad_w > 0 or self.pad_z > 0:
            x = F.pad(x, [self.pad_z // 2, self.pad_z - self.pad_z // 2, self.pad_w // 2, self.pad_w - self.pad_w // 2, self.pad_h // 2, self.pad_h - self.pad_h // 2]) ## Check
...
class BlockDecoder(object):
    ...
    def _decode_block_string(block_string):
        ...
        assert (('s' in options and len(options['s']) == 1) or
                (len(options['s']) == 3 and options['s'][0] == options['s'][1] and options['s'][1] == options['s'][2]))
    ...
    def _encode_block_string(block):
        ...
        args = [
            ...
            's%d%d' % (block.strides[0], block.strides[1],  block.strides[2]),
...
class EfficientNet3D(nn.Module):
    ...
    def _check_model_name_is_valid(cls, model_name, also_need_pretrained_weights=False):
        valid_models = ['efficientnet-b'+str(i) for i in range(8)]
